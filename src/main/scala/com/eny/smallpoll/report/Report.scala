 ...
 */
class Report(results:ResultRepository, answers:AnswerRepository, questions:QuestionRepository, surveys:SurveyRepository) {
  def result(from:Date, to:Date): List[SurveyReport] = {
    ...
    surveys.list(questionsMap.keys).map {
      survey => SurveyReport(from, to, survey.name, 0, questionsMap(survey.id.getOrElse(-1)))
    }
  }
}
